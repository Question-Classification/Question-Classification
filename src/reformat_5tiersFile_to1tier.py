...
def main():
    filename = "reformat_one_tier/cinq_tiers/ESLO1_INTPERS_407/ESLO1_INTPERS_407.TextGrid"
    ...
    tier3 = grid.get_tier_by_name(tiers_names[2])
    tier4 = grid.get_tier_by_name(tiers_names[3])
    tier5 = grid.get_tier_by_name(tiers_names[4])
        ...
        if elt.text != "question":
            ...
            for elt2 in tier2 :
                if elt.start_time == elt2.start_time and elt.end_time == elt2.end_time and "?" in elt2.text :
                                ...
                                question_list.append(res)
            for elt3 in tier3 :
                if elt.start_time == elt3.start_time and elt.end_time == elt3.end_time and "?" in elt3.text :
                                    start_time = elt.start_time
                                    end_time = elt.end_time
                                    label = elt.text
                                    label3 = elt3.text
                                    res = [start_time, end_time, label3]
                                    question_list.append(res)
            for elt4 in tier4 :
                if elt.start_time == elt4.start_time and elt.end_time == elt4.end_time and "?" in elt4.text:
                                    start_time = elt.start_time
                                    end_time = elt.end_time
                                    label = elt.text
                                    label4 = elt4.text
                                    res = [start_time, end_time, label4]
                                    question_list.append(res)
            for elt5 in tier5 :
                if elt.start_time == elt5.start_time and elt.end_time == elt5.end_time and "?" in elt5.text:
                                    start_time = elt.start_time
                                    end_time = elt.end_time
                                    label = elt.text
                                    label5 = elt5.text
                                    res = [start_time, end_time, label5]
                                    question_list.append(res)
    ...
    with open("reformat_one_tier/cinq_tiers/ESLO1_INTPERS_407/one_tier_ESLO1_INTPERS_407.TextGrid", "w", encoding="utf-8") as out:
        out.write(export)                
